...
function allUserMappingsResolved(unknown: ConsorcioUnknownValues, state: UserMappingState): boolean {
  return unknown.users.every((val) => {
    const s = state[val];
    if (!s) return false;
    if (s.mode === "skip") return true;
    if (s.mode === "create") return s.createName.trim().length > 0 && s.createEmail.trim().length > 0;
      ...
      {state.mode === "map" ? (
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            onChange={(e) => onChange({ ...state, createEmail: e.target.value })}
            placeholder="E-mail de login"
          />
        </div>
      ) : (
        <p className="text-[12px] text-slate-600">
          Linhas com este vendedor serão ignoradas na importação.
        </p>
      )}
    </div>
  );
...
export function ImportConsorcioDialog({
  open,
  onClose,
}: {
  open: boolean;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg border border-slate-200 bg-branco p-6 shadow-xl">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-[16px] font-semibold text-azul-profundo">Importar vendas (campanha)</h2>
            <p className="mt-1 text-[13px] text-slate-500">
              Planilha de consórcio com CLIENTE, VENDEDOR, CO VENDEDOR e TOTAL (ex.: consorcio.xlsx).
            </p>
          </div>
        </div>
        ...
        {!report ? (
          <div className="mt-5">
              ...
              <div className="mt-4">
                ...
                {preview.dataRowCount > 0 && (
                  <p className="mt-2 text-[12px] text-slate-500">
                    {preview.dataRowCount} venda(s) detectada(s) · {preview.sellerNames.length} nome(s) de vendedor na planilha
                  </p>
                )}
              </div>
            )}
            ...
            <p className="mt-2 text-[11px] text-slate-500">
              Cria leads fechados e registra vendas. Reimportar ignora duplicatas (mesmo cliente, valor, data e tipo).
            </p>
